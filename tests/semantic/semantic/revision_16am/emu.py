...
class Core:
    ...
    # ------------------------------------------------------------------
    # Phase 16AM: SCC side effects for the scalar logical family.
    #
    # The frozen revision_16t wrote the destination but never wrote SCC for
    # any of the instructions below, although the ISA says each of them sets
    # SCC.  The rule is uniform and is quoted from
    # `phase16r/isa/ref/rdna2_isa.txt`:
    #
    #   * bit/logical results   line 1891  "Bit/logical operations: 1 = result
    #                                      was not zero."
    #   * SOP2 opcode bodies    lines 5303/5307/5311/5315/5319/5323/5327/5331
    #                                      "D = S0 & S1;  SCC = (D != 0)." etc.
    #   * Table 14              lines 1991-2001  "Sets SCC? y" for
    #                                      {S_AND,S_OR,S_XOR}, {S_ANDN2,S_ORN2},
    #                                      S_LSHL, S_LSHR, S_ASHR.
    #
    # `_scc_of` is the single place that turns a destination value into SCC,
    # so every repaired handler below reads the same rule.
    # ------------------------------------------------------------------
    def _scc_of(self, d):
        """SCC = (result != 0) -- ISA line 1891 for bit/logical operations."""
        self.scc = 1 if (d & U32) != 0 else 0

    def op_s_and_b32(self, ins, ops):
        # ISA opcode 14 (line 5303-5304): D = S0 & S1; SCC = (D != 0).
        d = (self.sget(ops[1]) & self.sget(ops[2])) & U32
        self.sset(ops[0], d)
        self._scc_of(d)

    def op_s_or_b32(self, ins, ops):
        # ISA opcode 16 (line 5311-5312): D = S0 | S1; SCC = (D != 0).
        d = (self.sget(ops[1]) | self.sget(ops[2])) & U32
        self.sset(ops[0], d)
        self._scc_of(d)

    def op_s_xor_b32(self, ins, ops):
        # ISA opcode 18 (line 5319-5320): D = S0 ^ S1; SCC = (D != 0).
        d = (self.sget(ops[1]) ^ self.sget(ops[2])) & U32
        self.sset(ops[0], d)
        self._scc_of(d)

    def op_s_and_not1_b32(self, ins, ops):
        # The module's disassembly spells this `s_and_not1_b32`; the name in
        # this ISA revision is S_ANDN2_B32, opcode 20 (lines 5327-5328):
        # D = S0 & ~S1; SCC = (D != 0).  Same instruction, same rule.
        d = (self.sget(ops[1]) & ~self.sget(ops[2])) & U32
        self.sset(ops[0], d)
        self._scc_of(d)
    ...
    def _saveexec(self, dst, newmask):
        """D = EXEC_old; EXEC = newmask; SCC = (EXEC_new != 0).

        Phase 16AM: the SCC write lives here because the ISA gives the SAME
        rule for all five SAVEEXEC forms this revision implements.  ISA
        `phase16r/isa/ref/rdna2_isa.txt`:
          S_AND_SAVEEXEC_B32   (opcode 60) lines 6416-6418
          S_OR_SAVEEXEC_B32    (opcode 61) lines 6429-6431
          S_XOR_SAVEEXEC_B32   (opcode 62) lines 6439-6441
          S_ANDN2_SAVEEXEC_B32 (opcode 63) lines 6449-6451
          S_ANDN1_SAVEEXEC_B32 (opcode 68) lines 6502-6504
        each reading "EXEC_LO = <op>;  SCC = (EXEC_LO != 0)."

        SCC is taken from the MASKED new EXEC, because that is the value the
        emulator subsequently treats as EXEC and the value the ISA names.
        """
        old = self.exec_l
        self.sset(dst, old)
    ...
    def op_s_add_i32(self, ins, ops):
        ...
        ISA opcode 2, `phase16r/isa/ref/rdna2_isa.txt` lines 5252-5254:
            D.i = S0.i + S1.i;
            SCC = (S0.u[31] == S1.u[31] && S0.u[31] != D.u[31]). // signed
        overflow.
        Corroborated by Table 11 line 1902 ("S_ADD_I32 / S_ADD_U32 SOP2 y
        D = S0 + S1, SCC = overflow.") and by the prose at lines 1883-1885.

        The frozen revision wrote the destination and left SCC untouched.
        Both source reads happen before the write so that the `dst == src`
        aliasing sites in the module are unaffected.
        """
        ...
        sb = (b >> 31) & 1
        sd = (d >> 31) & 1
        self.scc = 1 if (sa == sb and sa != sd) else 0
    ...
    def op_s_sub_i32(self, ins, ops):
        ...
        ISA opcode 3, `phase16r/isa/ref/rdna2_isa.txt` lines 5261-5263:
            D.i = S0.i - S1.i;
            SCC = (S0.u[31] != S1.u[31] && S0.u[31] != D.u[31]). // signed
        overflow.
        Corroborated by Table 11 line 1904 and the prose at lines 1885-1890
        ("For subtract (AB), overflow = A and B have opposite signs and the
    ...
    def op_s_ashr_i32(self, ins, ops):
        # ISA opcode 34, lines 5389-5390:
        #   D.i = signext(S0.i) >> S1.u[4:0];  SCC = (D.i != 0).
        a = self.sget(ops[1])
        sh = self.sget(ops[2]) & 31
        d = a if sh == 0 else ((s32(a) >> sh) & U32)
        self.sset(ops[0], d)
        self._scc_of(d)

    def op_s_lshl_b32(self, ins, ops):
        # ISA opcode 30, lines 5373-5374:
    ...
    def op_s_lshl_b64(self, ins, ops):
        # ISA opcode 31, lines 5377-5378:
        #   D.u64 = S0.u64 << S1.u[5:0];  SCC = (D.u64 != 0).
        # Corroborated by Table 14 line 1997 ("S_LSHL_{B32,B64} SOP2 y").
        # `v` is read before `spair_set`, so the `dst == src` sites in the
        # module (`s_lshl_b64 s[8:9], s[8:9], 13`) are unaffected.
        v = self.spair(ops[1])
        sh = self.sget(ops[2]) & 63 if len(ops) > 2 else 0
        d = (v << sh) & ((1 << 64) - 1)
        self.spair_set(ops[0], d)
        self.scc = 1 if d != 0 else 0

    def op_s_abs_i32(self, ins, ops):
        # Table 11, `phase16r/isa/ref/rdna2_isa.txt` line 1925:
        #   "D.i = abs (S0.i). SCC=result not zero."
        d = abs(s32(self.sget(ops[1]))) & U32
        self.sset(ops[0], d)
        self._scc_of(d)
    ...
    def op_s_bfe_i32(self, ins, ops):
        ...
        ISA opcode 40, `phase16r/isa/ref/rdna2_isa.txt` lines 5420-5421:
            D.i = signext((S0.i >> S1.u[4:0]) & ((1 << S1.u[22:16]) - 1));
            SCC = (D.i != 0).

        OPERAND CONVENTION -- NOT the ISA's.  This handler decodes `ops[2]`
        as the field offset and `ops[3]` as the field width, i.e. a
        four-operand form.  The ISA packs both into ONE source operand
        (S1[4:0] = offset, S1[22:16] = width, lines 5411-5412 / 5417-5418),
        and the module's own disassembly uses that packed three-operand form
        (`s_bfe_i32 s18, s17, 0x80000`, i.e. width 8 at offset 0).  That
        divergence is NOT repaired here -- it is a decode defect, not an SCC
        defect, and changing it is outside this brief's scope.  Only the
        missing SCC side effect is added; the value D the handler already
        computed is left byte-for-byte as it was.  See the manifest entry
        `not_repaired` / `bfe_operand_encoding_divergence`.
        """
    ...
    def op_s_bfe_u32(self, ins, ops):
        ...
        ISA opcode 39, `phase16r/isa/ref/rdna2_isa.txt` lines 5414-5415:
            D.u = (S0.u >> S1.u[4:0]) & ((1 << S1.u[22:16]) - 1);
            SCC = (D.u != 0).

        Same four-operand decode caveat as `op_s_bfe_i32` above.
        """
        a = self.sget(ops[1]); o = self.sget(ops[2]) & 31; w = self.sget(ops[3]) & 31
    ...
    def op_s_min_i32(self, ins, ops):
        """S_MIN_I32 -- SCC = (S0 < S1), signed.

        ISA opcode 6, `phase16r/isa/ref/rdna2_isa.txt` lines 5275-5276:
            D.i = (S0.i < S1.i) ? S0.i : S1.i;
            SCC = (S0.i < S1.i).
        Corroborated by Table 11 line 1916 ("D = (S0 < S1) ? S0 : S1.
        SCC = 1 if S0 was min.").

        Note the comparison is on the ORIGINAL source pair, not on the
        result: a min of (5, 5) leaves SCC = 0.
        """
        a = self.sget(ops[1])
        b = self.sget(ops[2])
        self.sset(ops[0], min(s32(a), s32(b)) & U32)
        self.scc = 1 if s32(a) < s32(b) else 0
