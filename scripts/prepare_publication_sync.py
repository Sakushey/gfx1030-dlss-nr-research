...
A mapping whose published copy is a CURATED DERIVATIVE -- one carrying
post-review additions, so that no mechanical transform reproduces it from the
private original -- is declared with the `curated derivative: ...` value in
`modifications`. The helper recognizes that declaration and REFUSES to invent
bytes for it: while the private source still matches the reviewed revision
there is nothing to sync, and as soon as it does not, the mapping blocks and a
human must decide. There is no third behaviour.
...
# Identity patterns for the private tree this payload came from. A payload
# destined for publication must not name the private working directory or the
# LIVE private phase. Historical private-phase provenance tokens (`phase16o`,
# `phase16j`, ...) are already all over the approved published tree and are
# deliberately NOT matched: rejecting them would reject the known-good case.
# LIVE_PRIVATE_PHASE_FAMILY is a maintenance point -- advance it as the private
# series advances, exactly as a year is advanced in a copyright line.
LIVE_PRIVATE_PHASE_FAMILY = "phase16a"

IDENTITY_PATTERNS = {
    "private_project_dirname": re.compile(
        rb"(?i)gfx1030_soft_wmma_phase1"
    ),
...
# --- what a `modifications` value is allowed to say --------------------------
#
# Every value below is a claim about the RELATION BETWEEN TWO BYTE STREAMS.
# The value the helper accepts must be TRUE of the bytes actually measured; a
# claim that is untrue is worse than an unrecognized one, because it moves a
# falsehood into the validated column.
#
#   none                                 source bytes == public bytes
#   line endings normalised CRLF -> LF   CRLF->LF ALONE maps source to public
#   UTF-8 BOM removed                    dropping a leading UTF-8 BOM ALONE does
#   UTF-8 BOM added                      adding a leading UTF-8 BOM ALONE does
#   CURATED_DERIVATIVE                   NO mechanical transform maps one to
#                                        the other: the published copy is a
#                                        curated derivative of the original
#
# The first four are mechanisable, so `transform()` implements them and
# `curated-derivative` is NOT among them. See CURATED_DERIVATIVE below.
...
#: The subset whose transform can actually be applied. A value in
#: RECOGNIZED_MODIFICATIONS but not here is recognized *as a refusal*.
MECHANICAL_MODIFICATIONS = RECOGNIZED_MODIFICATIONS - {CURATED_DERIVATIVE}
...
class NotMechanicallySerialisable(RuntimeError):
    """Raised for a mapping that has no mechanical transform.

    A curated derivative cannot be regenerated from its private original.
    There is no byte payload this helper is entitled to write, so refusing is
    the only honest outcome: returning the source bytes unchanged would assert
    that the public copy equals the private original, which the measured facts
    contradict, and returning anything else would be an invention.
    """
...
def transform(data: bytes, modification: str) -> bytes:
    """Apply a declared mechanical transform.

    Raises `NotMechanicallySerialisable` for a recognized declaration whose
    transform does not exist, and `RuntimeError` for a name outside the
    vocabulary. Never returns a best guess.
    """
    if modification == "none":
        return data
    if modification == "line endings normalised CRLF -> LF":
        return data.replace(b"\r\n", b"\n")
    if modification == "UTF-8 BOM removed":
        return data[3:] if data.startswith(UTF8_BOM) else data
    if modification == "UTF-8 BOM added":
        return data if data.startswith(UTF8_BOM) else UTF8_BOM + data
    if modification == CURATED_DERIVATIVE:
        raise NotMechanicallySerialisable(
            "the published copy of this file is a curated derivative: no "
            "transform maps the private original onto it, so it cannot be "
...
def scan_candidate(rel: str, data: bytes) -> list[str]:
    ...
    for name, rx in IDENTITY_PATTERNS.items():
        if rx.search(data):
            findings.append(
                f"{rel}: private-identity content [{name}]"
            )
    return findings
...
def scan_manifest(
    ...
    `manifest` may be supplied explicitly so a caller can scan a candidate
    manifest that is not (or not yet) the one on disk. The default reads the
    manifest from disk.
    """
    ...
    for entry in files:
        ...
        except NotMechanicallySerialisable as exc:
            # The mapping is declared curated, so there is no transform to
            # apply -- and the private source has now moved away from the
            # reviewed revision. Refusing is the whole point: nothing may be
            # written, and a human must decide what the published copy becomes.
            blockers.append(
                f"{dst_rel}: mapping is declared a curated derivative (no "
                f"mechanical transform) and the private source has changed "
                f"since the reviewed revision was recorded; this mapping needs "
                f"a curated re-publication and a manifest update ({exc})"
            )
            continue
    ...
    result = {
        ...
        "blockers": blockers,
        "curated_derivative_pairs": curated,
    }
...
def main() -> int:
    ...
    out: dict[str, Any] = {
        ...
        "curated_derivative_pairs": result["curated_derivative_pairs"],
        "missing_source_files": missing,
        "blockers": blockers,
        "changes": [
