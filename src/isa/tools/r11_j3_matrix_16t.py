#!/usr/bin/env python3
# ===========================================================================
# PHASE 16T COPY of phase16s/isa/tools/r11_j3_matrix_16s.py, which is itself
# the PHASE 16S copy of phase16r/isa/tools/r11_j3_matrix.py.
#
# THE FOUR 16T CHANGES ARE LISTED IN THIS MODULE'S DOCSTRING BELOW.
# The comment block that follows THIS banner is the 16S text verbatim and
# describes the 16S copy; it is kept because the lineage matters, and it is
# NOT a description of this file.  Read the docstring for this file's changes.
# ===========================================================================
# ===========================================================================
...
# ===========================================================================
"""Phase 16T COPY of phase16s/isa/tools/r11_j3_matrix_16s.py.

Byte-for-byte the 16S tool except for four changes, all recorded here:
  * OUT_REL / LOG_REL point at phase16t/isa, so this copy writes NOTHING into
    phase16s/isa (which is 16S evidence and is not touched);
  * S1_ORACLE_REL points at phase16t/isa/ISA_ORACLE_16T.json, the same 146
    vectors with the two ill-posed MIX families' expectations recomputed from
    the state each vector actually sets up (see build_isa_oracle_16t.py);
  * the two 16L oracle subprocesses are routed through
    phase16t/tools/run_under_rev.py so they measure the SAME source revision
    this matrix does, instead of the live tree;
  * `HERE`/`ISA`/`LOGS`/`ROOT` are derived from this file's own location, so
    placing the copy at the same depth repoints them automatically.

Every other line is the 16S text, so the 16L / M2 / Q1 measurements this tool
performs are the same measurements, run against a different revision.
...
R9_REL = "phase16t/isa/R9_DIVISION_SEMANTICS_16T.json"
...
OUT_REL = "phase16t/isa/J3_INSTRUCTION_CONFORMANCE_16T.json"
LOG_REL = "phase16t/isa/logs/r11_matrix_16t.txt"
...
#: PHASE 16T: the `resolution` block of the R9 artifact this run loaded, set
#: in `main` before any classification happens.  It is the ONLY thing that can
#: lift the three `v_div_*` rows off BLOCKED, and it only lifts them when the
#: oracle MEASURED full lane-instance agreement -- the branch above reads the
#: measured counts, not a hand-written boolean.
R9_RESOLUTION = None
...
def run_sixteenl(script, label, out_name, do_run=True):
    ...
    # PHASE 16T: the 16L verifiers must measure the SAME source revision
    # this matrix does.  Routed through the revision wrapper; without it the
    # children would silently load the live tree and the sub-results would be
    # live-tree measurements filed under a revision run.
    _wrap = os.path.join(ROOT, "phase16t", "tools", "run_under_rev.py")
    _rev = os.environ.get("P16T_REV", "revision")
    cmd = [sys.executable, _wrap, "--rev", _rev, "--script",
           os.path.join(ROOT, script), "--", "--label", label,
           "--out", out_abs]
...
S1_ORACLE_REL = "phase16t/isa/ISA_ORACLE_16T.json"
...
def classify(row, evid, r9_verdict, adjudication=None):
    ...
    if row["mnemonic"] in DIV_MNEMONICS and r9_verdict:
        # PHASE 16T.  The 16S body cited R9's blocked verdict unconditionally.
        # Phase 16T re-ran the whole-sequence oracle against the successor
        # semantic revision; when that artifact MEASURES full agreement on
        # this fixture's lane-instances, the row is verified by that oracle
        # rather than blocked by a verdict that is no longer current.  The
        # general gap U1c is untouched, still open, and not reached here.
        res = R9_RESOLUTION or {}
        if res.get("j3_clauses_resolved"):
            return ("VERIFIED_INDEPENDENTLY",
                    ("whole-sequence equivalence: mode %s agrees with the "
                     "independently computed, correctly rounded reference on "
                     "%d of %d J3 lane-instances over %d distinct PCs under "
                     "semantic revision %s.  Basis: %s -- explicitly not %s. "
                     "U1 and U1b are RESOLVED_FOR_J3; the general gap U1c "
                     "remains OPEN and is not reached by this fixture."
                     % (res.get("mode"), res.get("lane_instances_agree"),
                        res.get("lane_instances_total"),
                        res.get("distinct_pcs"),
                        str(res.get("semantic_revision_hash"))[:16],
                        res.get("basis"), res.get("explicitly_not"))), None)
...
def main(argv=None):
    ...
    r9 = load(R9_REL)
    global R9_RESOLUTION
    R9_RESOLUTION = r9.get("resolution")
