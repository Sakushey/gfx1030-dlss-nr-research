...
def check_emulator_identity():
    """Criterion: the emulator ACTUALLY LOADED must be the one this run names.

    PHASE 16T.  The 16S body required the frozen 16O `emu.py`.  Phase 16T
    deliberately runs a SUCCESSOR revision, so the required identity is the
    revision tree named by `P16T_REV_TREE` (default: the 16T revision), and
    the check reads `emu.__file__` from the imported module -- not from a path
    this tool typed -- exactly as before.  The parent-revision identity is
    still MEASURED and recorded beside it, so the artifact says which tree ran
    and what the parent would have been, rather than dropping the check.

    A run under `--rev live` keeps the original requirement.
    """
    _bootstrap_emulator_path()
    import emu as EMU
    path = os.path.abspath(EMU.__file__)
    actual = sha256_file(path)
    rev_tree = os.environ.get("P16T_REV_TREE")
    want = os.path.abspath(rev_tree) if rev_tree else None
    rec = {
        "loaded_module_file": path,
        "expected_relative": EMU_REL,
        "frozen_path": os.path.abspath(EMU_PATH),
        "is_the_frozen_path": path == os.path.abspath(EMU_PATH),
        "frozen_sha256": sha256_file(EMU_PATH),
        ...
        "revision_tree_required": want,
        "loaded_from_the_required_revision_tree":
            bool(want) and path.startswith(want + os.sep),
    }
    if want:
        # a revision run: the frozen tree must NOT be what loaded, and the
        # named tree must be
        rec["identity_ok"] = (rec["loaded_from_the_required_revision_tree"]
                              and not rec["is_the_frozen_path"])
    else:
        rec["identity_ok"] = (rec["is_the_frozen_path"]
                              and actual.startswith(EMU_SHA256_BRIEF[:32])
                              and actual.endswith(EMU_SHA256_BRIEF[-8:]))
    rec["note"] = (
        "The Phase 16S brief's literal is %d hex characters, which is not a "
        "SHA-256 length; it is reported verbatim. Under a 16T revision run the "
        "required identity is the revision tree and the frozen identity is "
        "recorded beside it." % len(EMU_SHA256_BRIEF))
...
S = _il.import_module("seq_oracle_16t")
