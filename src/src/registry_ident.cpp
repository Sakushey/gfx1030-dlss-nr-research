...
bool gate_from_cfg(const char* name)
{
    // Resolve this module's own directory (bridge DLL) without touching
    // anything outside KERNEL32. Env overrides handled by the callers.
    HMODULE self = nullptr;
    if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                                GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                            (LPCSTR)&gate_from_cfg, &self)) {
        return false;
    }
    char path[MAX_PATH];
    ...
    std::strncat(path, "dlssnr16.cfg", sizeof(path) - std::strlen(path) - 1);
    ...
    char buf[4096];
    DWORD rd = 0;
    bool ok = ReadFile(f, buf, sizeof(buf) - 1, &rd, nullptr) && rd > 0;
    CloseHandle(f);
    if (!ok) {
        return false;
    }
    buf[rd] = '\0';
    // match a full line: NAME=1  (CR/LF tolerant, case-sensitive key)
    const size_t klen = std::strlen(name);
    for (char* p = buf; *p;) {
        char* eol = std::strchr(p, '\n');
        size_t llen = eol ? (size_t)(eol - p) : std::strlen(p);
        char line[128];
        if (llen >= sizeof(line)) {
            llen = sizeof(line) - 1;
        }
        std::memcpy(line, p, llen);
        line[llen] = '\0';
        if (llen > 0 && line[llen - 1] == '\r') {
            line[llen - 1] = '\0';  // CRLF tolerance
        }
        if (std::strncmp(line, name, klen) == 0 && line[klen] == '=' &&
            std::strcmp(line + klen + 1, "1") == 0) {
            return true;
        }
        if (!eol) {
            break;
...
bool substitution_enabled()
{
    ...
    // Deployment-config fallback (dlssnr16.cfg next to this module).
    return gate_from_cfg("USE_GFX1030_FATBIN");
}
