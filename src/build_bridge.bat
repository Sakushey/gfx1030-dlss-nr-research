@echo off
rem Phase 14D9-D entry-fixed bridge builds (host-only; MSVC 2022 Build Tools).
...
rem Everything else (29-export ABI, backend path, fail-closed identity,
rem launch gate) is unchanged from the Phase 10/13 sources.
...
echo [3/5] smoke test executable
cl %CFLAGS% src\hip_bridge_smoke.cpp /link /OUT:hip_bridge_smoke.exe || exit /b 1

echo [4/5] registration smoke test executable
